...
class GalleryboxAsset extends StatefulWidget {
  final String postAsset;
  const GalleryboxAsset({super.key,required this.postAsset});

  @override
  State<GalleryboxAsset> createState() => _GalleryboxAssetState();
}

class _GalleryboxAssetState extends State<GalleryboxAsset> {
  late final PageController _controller;
   List<Widget> images=[];
  ...
  void initState() {
    _controller = PageController();
    images.add(Image.asset("assets/images/${widget.postAsset}"));
    images.add(const Center(child:  Text("Hello1")));
    images.add(const Center(child:  Text("Hello2")));
    images.add(const Center(child:  Text("Hello3")));
    super.initState();
